...
double sum_ex_dispatch = 0;
double n_ex_dispatch   = 0;
double sum_ex_add = 0;
double n_ex_add   = 0;
time_t start_ex;
time_t end_ex;
...
static int sstf_dispatch(struct request_queue *q, int force){
	...
	time(&end);
	sum_ex_dispatch += difftime(end, start);
	n_ex_dispatch   += 1;
	if(closest_rq){
...
static void sstf_exit_queue(struct elevator_queue *e)
{
	...
	double avg_add = sum_ex_add / n_ex_add;
	double avg_dispatch = sum_ex_dispatch / n_ex_dispatch;
	double ex_time = difftime(end_ex, start_ex);
